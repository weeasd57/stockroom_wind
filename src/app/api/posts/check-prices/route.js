...
export async function POST(request) {
  ...
  try {
    ...
    const { data: allPosts, error: allPostsError, status: allPostsStatus } = await supabase
      .from('posts')
      .select('id, closed')
      .eq('user_id', userId);
    ...
    console.log(`[SUBSCRIPTION_INFO] Final usage calculation:`, {
      ...
      usageCount,
      remainingChecks,
      priceCheckLimit: updatedSubscriptionInfo?.price_check_limit,
      priceChecksUsed: updatedSubscriptionInfo?.price_checks_used
    });
